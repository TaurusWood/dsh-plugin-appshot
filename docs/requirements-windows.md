...
用户已经在 DSH 中打开一个目标会话，随后切换到其他桌面应用查看内容。当用户看到不理解的界面、错误或现象时，将鼠标放在目标窗口的可见区域，同时按下左右 Ctrl。目标窗口必须完整位于鼠标所在显示器内；插件捕获该窗口并将图片加入触发前由 DSH Client 明确上报的目标会话 Composer，等待用户补充文字后一起发送。
...
| 目标窗口 | Hook 触发时采样鼠标物理坐标，工作线程首次调度时在该固定坐标上解析的可捕获顶层窗口 |
| 目标显示器 | 触发瞬间鼠标指针所在显示器 |
| 跨屏 | 禁止跨屏截图、拼接或静默裁剪 |
...
- 目标窗口在鼠标所在显示器内完整显示，且至少有一块可见区域供用户放置鼠标；
...
[Hook 抓取坐标，工作线程同步隐藏旧通知]
                 │
                 ▼
[使用触发时 cursorPoint 解析 monitor / HWND，校验目标并保存 visible backup]
                 │
...
- 以触发瞬间的鼠标坐标为唯一入口，不在后续异步线程重新读取鼠标位置；
- Hook 不在回调内执行窗口查询；HWND 在工作线程首次调度时解析。若目标在这个短暂窗口内关闭、移动或层级改变，以保存坐标上当时可解析表面为准；不改用新鼠标位置，无合法目标则失败；
- 在执行 `WindowFromPoint` 之前，Native 必须同步隐藏自身所有活动通知窗口，防止误命中自身；
- 从鼠标下的子窗口或控件规范化到可捕获的顶层窗口（先用 `GA_ROOT`）；
- 排除：
  - DSH 自身窗口；
  - Native Agent 的提示窗口；
  - 桌面、任务栏、开始菜单等系统 Shell 表面；
  - 隐藏、最小化、Cloaked 或无有效可视边界的窗口；
- 鼠标位于 Tooltip、菜单、浮动工具窗等独立顶层表面时，识别为该独立浮层而不是其所属主窗口。
