...
Registers the `dsh_plugin_appshot` tool via `defineTool()`.
...
dsh --profile my-profile       # watch for: [dsh-plugin-appshot] registered "dsh_plugin_appshot" — listed=true
```
...
```sh
dsh --profile my-profile --dump-config | grep dsh-plugin-appshot   # 配置层含本行
dsh plugin --profile headless add ./dsh-plugin-appshot
